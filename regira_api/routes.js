...
router.post("/issues", checkToken, async (req, res, next) => {
  try {
    ...
    const { title, issue_type, priority, state, projectId, authorId } =
    ...
    const item = await Issue.create(req.body); // Crea un nou issue amb les dades rebudes
...
router.post("/comments/issues/:issueId", checkToken, async (req, res) => {
  try {
    ...
    if (
        ...
        return res
          .status(400)
          .json({ error: "title, comment son requerits" }); // Retorna error 400 si no es proporcionen el nom, email o contrasenya
      }
      if (!issue) {
